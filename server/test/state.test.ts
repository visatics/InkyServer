...
describe("resolveAction precedence (via applyButton/describeAction)", () => {
  it("uses the device default when no override exists", () => {
    ...
    expect(applyButton(cfg, { screen: 2, idx: 1 }, "A")).toEqual({ screen: 1, idx: 0 });
  });
...
describe("goto resets provider keys", () => {
  ...
  it("goto the current screen still resets to initial state", () => {
    expect(applyButton(cfg, { screen: 1, idx: 2 }, "A")).toEqual({ screen: 1, idx: 0 });
  });
});
...
describe("cycleValue", () => {
  ...
  it("cycle via button E on the debug screen", () => {
    expect(applyButton(cfg, { screen: 3, mode: "light" }, "E")).toEqual({ screen: 3, mode: "dark" });
    expect(applyButton(cfg, { screen: 3, mode: "blue" }, "E")).toEqual({ screen: 3, mode: "light" });
  });
});

describe("advanceIdx", () => {
  it("wraps forward using screen 1's N=3", () => {
    expect(advanceIdx(cfg, { screen: 1, idx: 2 }, "next").idx).toBe(0);
    expect(advanceIdx(cfg, { screen: 1, idx: 0 }, "next").idx).toBe(1);
  });

  it("wraps backward using screen 1's N=3", () => {
    expect(advanceIdx(cfg, { screen: 1, idx: 0 }, "prev").idx).toBe(2);
  });

  it("uses screen 2's smaller N=2", () => {
    expect(advanceIdx(cfg, { screen: 2, idx: 1 }, "next").idx).toBe(0);
    expect(advanceIdx(cfg, { screen: 2, idx: 0 }, "prev").idx).toBe(1);
  });

  it("is identity on non-slideshow screens", () => {
    const state = { screen: 3, mode: "dark" };
    expect(advanceIdx(cfg, state, "next")).toEqual(state);
  });

  it("normalises an out-of-range idx via modulo", () => {
    expect(advanceIdx(cfg, { screen: 1, idx: 7 }, "next").idx).toBe(2); // 7 mod 3 = 1 -> 2
  });
});

describe("applyRefresh", () => {
  it("advances a sequential slideshow", () => {
    expect(applyRefresh(cfg, { screen: 1, idx: 0 }).idx).toBe(1);
    expect(applyRefresh(cfg, { screen: 1, idx: 2 }).idx).toBe(0);
  });

  it("picks a non-repeating index for a random slideshow (N>1)", () => {
    // Screen 2 is random with N=2; RNG insists on the current index 0
    expect(applyRefresh(cfg, { screen: 2, idx: 0 }, rnd(0.1)).idx).toBe(1);
    expect(applyRefresh(cfg, { screen: 2, idx: 1 }, rnd(0.9)).idx).toBe(0);
  });

  it("is identity for debug", () => {
    const state = { screen: 3, mode: "blue" };
    expect(applyRefresh(cfg, state)).toEqual(state);
  });
});

describe("initialState / first boot", () => {
  it("slideshow screens start at idx 0", () => {
    expect(initialState(cfg, 1)).toEqual({ screen: 1, idx: 0 });
    expect(initialState(cfg, 2)).toEqual({ screen: 2, idx: 0 });
  });

  it("the debug screen starts in light mode", () => {
    expect(initialState(cfg, 3)).toEqual({ screen: 3, mode: "light" });
  });

  it("unknown ordinals fall back to the default screen", () => {
    expect(initialState(cfg, 99)).toEqual({ screen: 1, idx: 0 });
  });

  it("first boot does NOT advance: no screen param -> inbound null", () => {
    expect(parseState(cfg, {}).inbound).toBeNull();
    expect(parseState(cfg, { idx: "2" }).inbound).toBeNull(); // idx without screen is still a boot
  });
});

describe("parseState sanitisation", () => {
  it("unknown screen ordinals collapse to a reset (inbound null)", () => {
    expect(parseState(cfg, { screen: "99", idx: "1" }).inbound).toBeNull();
  });

  it("non-numeric idx falls back to 0", () => {
    expect(parseState(cfg, { screen: "1", idx: "abc" }).inbound).toEqual({ screen: 1, idx: 0 });
  });

  it("out-of-range idx is normalised via modulo", () => {
    expect(parseState(cfg, { screen: "1", idx: "7" }).inbound).toEqual({ screen: 1, idx: 1 });
    expect(parseState(cfg, { screen: "1", idx: "-1" }).inbound).toEqual({ screen: 1, idx: 2 });
  });

  it("passes the button through, including unknown ones", () => {
    expect(parseState(cfg, { screen: "1", idx: "0", button: "Z" }).button).toBe("Z");
    expect(parseState(cfg, { screen: "1", idx: "0" }).button).toBeUndefined();
  });

  it("keeps mode on the debug screen and drops idx", () => {
    const { inbound } = parseState(cfg, { screen: "3", mode: "dark", idx: "1" });
    expect(inbound).toEqual({ screen: 3, mode: "dark" });
  });

  it("an invalid mode survives parsing but cycles from values[0]", () => {
    const { inbound } = parseState(cfg, { screen: "3", mode: "purple" });
    expect(applyButton(cfg, inbound!, "E")).toEqual({ screen: 3, mode: "light" });
  });

  it("the kitchen sink (screen=99, idx=abc, mode=purple, button=Z) never throws", () => {
    const parsed = parseState(cfg, { screen: "99", idx: "abc", mode: "purple", button: "Z" });
    ...
    expect(applyButton(cfg, initialState(cfg, 1), "Z")).toEqual({ screen: 1, idx: 0 });
  });
});

describe("renderStateFor (cache-key subset)", () => {
  it("slideshow keys on idx only, clamped", () => {
    expect(renderStateFor(getScreen(cfg, 1), { screen: 1, idx: 7 })).toEqual({ idx: 1 });
    expect(renderStateFor(getScreen(cfg, 1), { screen: 1 })).toEqual({ idx: 0 });
  });

  it("debug keys on mode only, defaulting to light", () => {
    expect(renderStateFor(getScreen(cfg, 3), { screen: 3, mode: "dark" })).toEqual({ mode: "dark" });
    expect(renderStateFor(getScreen(cfg, 3), { screen: 3 })).toEqual({ mode: "light" });
  });
});
